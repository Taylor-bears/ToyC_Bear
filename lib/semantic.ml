...
let rec check_expr env = function
  ...
  | EBinOp (op, e1, e2) ->
      ...
      | Add | Sub | Mul | Div | Mod ->
          ...
          else raise (Semantic_error "算术运算需要整数操作数")
      | Lt | Le | Gt | Ge | Eq | Neq ->
          if t1 = TInt && t2 = TInt then TBool
          else raise (Semantic_error "比较运算需要整数操作数")
      | And | Or ->
          if (t1 = TBool || t1 = TInt) && (t2 = TBool || t2 = TInt) then TBool
          else raise (Semantic_error "逻辑运算需要布尔或整数操作数")
      end
  | ECall (id, args) ->
...
let check_comp_unit (CUnit funcs) =
  ...
  try
    ...
    | TFunc (TInt, []) -> global_env  (* 正确：int main() *)
    | _ -> raise (Semantic_error "main 必须定义为 int main()")
  with Semantic_error _ ->
    raise (Semantic_error "缺少 int main() 函数")